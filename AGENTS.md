...
pytest                          # run all 101 tests (~6s)
pytest --cov=startup_valuation  # with coverage (88%)
ruff check .                    # lint
mypy src/startup_valuation      # type check
```

CI runs `mypy` + `pytest --cov=... -v` on Python 3.10/3.11/3.12. Docs build with `mkdocs build --strict`.

## Architecture

- `src/startup_valuation/` — 14 modules, 100+ functions. All return `ValuationResult` (never bare numbers).
- `src/startup_valuation/types.py` — `ValuationResult`, `Distribution`, `Scenario`, `SensitivityResult` dataclasses.
- `mcp_server/server.py` — FastMCP server with 60+ tools. Run: `cd mcp_server && python server.py`. Requires `pip install -e ".[mcp]"`.
...
- `specs/` — SDD artifacts for valuation-engine and model-enhancement-publish projects.
...
- `scipy.integrate.quad` used for continuous expected value numerical integration.
